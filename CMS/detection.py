...
def detect(frame, mask, lower, upper):
    ...
    # Apply mask to image
    hsv = cv2.bitwise_and(hsv, hsv,
                          mask=mask)  # syntax cv2.bitwise_and(img_array_1, img_array_2, mask) much like intersection
    # in Venn Diagrams

    # Preparing the mask to overlay
    thresh = cv2.inRange(hsv, lower,
                         upper)  # inRange function is thresholding the HSV image for a range of color within the
    # boundaries (lower,upper) thresh is now an image in HSV format converted from BGR frame captured,
    # only its intersection with mask.png taken and the taken part is thresholded within the range (lower,upper)

    # Perform a series of erosions and dilations to remove any small blobs of noise from the thresholded image
    # all operations are done on the previously configured "thresh" image
    thresh = cv2.erode(thresh, None,
                       iterations=1)  # Syntax: cv2.erode(image, kernel = a structure according to which the erosion is governed(None taken), iterations = number of times the image is to be eroded)
    thresh = cv2.dilate(thresh, None,
                        iterations=1)  # Syntax: cv2.dilate(image, kernel = a structure according to which the erosion is governed(None taken) ,iterations = number of times the image is to be eroded)
